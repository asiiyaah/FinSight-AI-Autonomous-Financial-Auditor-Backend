...
def calculate_cashflow(statement_id):
    """
Calculate core cashflow metrics for a statement.
...
- savings_rate
    ...
    total_credit = Transaction.objects.filter(
        ...
        transaction_type="credit").aggregate( total=Sum("amount")
                                            )["total"] or 0
    
    total_debit = Transaction.objects.filter(
        statement_id=statement_id,
        transaction_type="debit").aggregate(total=Sum("amount")
                                            )["total"] or 0
...
Helps identify accidental double charges or suspicious billing.
    """
    transactions=Transaction.objects.filter(statement_id=statement_id,transaction_type="debit")
    groups={}
...
- Detects unusual spending, not fraude
